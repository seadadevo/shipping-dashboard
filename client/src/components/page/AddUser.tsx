...
export function AddUser({ onSave }: AddUserProps) {
  ...
  const [userType, setUserType] = useState<"merchant" | "courier" | "employee">(
    "employee"
  );
  ...
  const handleGovernorateChange = (governorateId: string) => {
    ...
    const filteredCities = cities.filter(
      (city) =>
        city.governorate._id === governorateId && city.isActive !== false
    );
  ...
  const handleSubmit = async () => {
    ...
    if (userType === "courier") {
      requiredFields.push("address", "governorateId", "cityId");
    }
    ...
    try {
      ...
      const selectedGov = governorates.find(
        (g) => g._id === formData.governorateId
      );
      ...
      const newUser = {
        ...
        rejectionFeePercentage: formData.rejectionFeePercentage || undefined,
    ...
    <div className="space-y-8">
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>اختر نوع الحساب المراد إنشاؤه</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex gap-2 space-x-4 space-x-reverse">
            ...
            >
              <Store
                className={`h-4 w-4 mr-2 ${
                  userType === "employee" ? "text-white" : "text-gray-500"
                }`}
              />
              موظف
            </Button>
            ...
            >
              <Store
                className={`h-4 w-4 mr-2 ${
                  userType === "merchant" ? "text-white" : "text-gray-500"
                }`}
              />
              تاجر
            </Button>
            ...
            >
              <User
                className={`h-4 w-4 mr-2 ${
                  userType === "courier" ? "text-white" : "text-gray-500"
                }`}
              />
              مندوب توصيل
            </Button>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleInputChange("pickupCost", e.target.value)
                  }
                  placeholder="0.00"
                  className="text-right"
                  required
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleInputChange("rejectionFeePercentage", e.target.value)
                  }
                  placeholder="0.0"
                  className="text-right"
                  required
      ...
      <Card className="border-l-4 border-orange-500 bg-yellow-50 shadow-sm">
        ...
        <CardContent className="text-orange-700">
          <ul className="list-disc list-inside space-y-2 text-sm pr-4">
            <li>
              تأكد من صحة البريد الإلكتروني حيث سيتم إرسال تفاصيل الحساب إليه.
            </li>
            <li>كلمة المرور يجب أن تكون قوية وتحتوي على أحرف وأرقام ورموز.</li>
            <li>رقم الهاتف مطلوب للتواصل وإرسال الإشعارات.</li>
            {userType === "merchant" && (
              <>
                <hr className="my-3 border-orange-200" />
                <li>
                  **تكلفة الاستلام الخاص:** ستُطبق على جميع طلبات هذا التاجر
                  بدلاً من التكلفة العامة.
                </li>
                <li>
                  **نسبة تحمل الرفض:** تحدد النسبة المئوية للتكلفة التي يتحملها
                  التاجر عند رفض العميل للطلب.
                </li>
              </>
            )}
          </ul>
